...
import { HandCoins, Plus, Trash2, User, FileText } from 'lucide-react';

export default function DebtsOwedByUsPage() {
  const { user } = useAuth();
  ...
  const debts = useLiveQuery(() => db.debtsOwedByUs.toArray(), []);
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من حذف هذا الحساب؟')) {
      await db.debtsOwedByUs.delete(id);
    }
  };
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">حسابات في ذمتي</h1>
          <p className="text-slate-500 text-sm mt-1">إدارة الديون المستحقة علي الشركة</p>
        </div>
      ...
      <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-6 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-amber-100 text-sm">إجمالي المبالغ في ذمتي</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {(debts || []).length === 0 ? (
          <div className="p-12 text-center">
            <HandCoins className="w-16 h-16 mx-auto mb-4 text-slate-300" />
